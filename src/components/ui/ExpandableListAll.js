...
function TodoListModal({
  ...
  //console.log("TodoListModal props:", { listType, list, type, groupId, user });
  ...
  // Move useMemo to the top level, before any early returns
  const optionsMap = useMemo(() => {
    ...
    list.todoParameters.forEach((param) => {
      ...
      if (Array.isArray(param.options)) {
        // If options is an array, check if it contains a single string that needs splitting
        if (
  ...
  // Initialize advanced todo data based on todoParameters
  useEffect(() => {
    if (list?.isAdvanced && list?.todoParameters) {
      const initialData = {};
      list.todoParameters.forEach((param) => {
        initialData[param.id] = param.value || ""; // Use null or empty string as default
      });
  ...
  const addTodoToList = async () => {
    ...
    try {
      ...
      if (list?.todoParameters) {
        // Create an array of parameter objects with values
        const parametersArray = list.todoParameters.map((param) => ({
          id: param.id,
          label: param.label,
          type: param.type,
          value:
            advancedTodoData[param.id] || (param.type === "number" ? null : ""),
        ...
        todoData = {
          ...
          parameters: parametersArray, // Store as array of parameters
        };
      } else {
        todoData = {
          text: newTodoText,
      ...
      if (list?.isAdvanced) {
        const initialData = {};
        list.todoParameters.forEach((param) => {
          initialData[param.id] = ""; // Reset to empty
        });
        setAdvancedTodoData(initialData);
      } else {
        setNewTodoText("");
  ...
  const handleAddTodo = async (e) => {
    ...
    if (listType === "listsAdvanced") {
      ...
      if (allRequiredFieldsFilled && hasAnyValue) {
        ...
        // Show an error for required fields
  ...
  // Updated handleAdvancedInputChange function
  const handleAdvancedInputChange = (fieldId, value) => {
    ...
    if (parameter?.type === "number") {
      // For number inputs, convert to number if valid, otherwise keep as string
      processedValue =
  ...
  const handleEditTodo = async (todoId, newData) => {
    try {
      ...
      if (list?.isAdvanced || listType === "listsAdvanced") {
        // For advanced todos, ensure we're updating the parameters array correctly
        ...
        updateData = {
          parameters: newData.map((param) => ({
            ...
            label: param.label,
            type: param.type,
            value: param.value || (param.type === "number" ? null : ""),
            required: param.required || false,
          })),
          updatedAt: new Date().toISOString(),
        };
      ...
      // Optionally, you could add error handling feedback to the user here
    }
  };
