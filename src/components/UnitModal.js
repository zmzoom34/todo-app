...
const UnitModal = ({ isOpen, onClose }) => {
  ...
  const handleAddUnit = async () => {
    ...
    await setDoc(doc(db, "units", newUnitId), { name: newUnit });
    setNewUnit("");
    setNewUnitId("");
    fetchUnits();
  };
  ...
  const handleUpdateUnit = async () => {
    ...
    if (editingUnit.id === editId) {
      ...
      await deleteDoc(doc(db, "units", editingUnit.id));
      await setDoc(doc(db, "units", editId), { name: editText });
    }
  ...
  return (
    ...
    >
      ...
      >
        ...
        <h2 className="text-xl font-bold mb-4">Birimleri Yönet</h2>
        <div className="flex flex-col gap-2 mb-4">
          <Input
            type="text"
            value={newUnitId}
            onChange={(e) => setNewUnitId(e.target.value)}
            placeholder="Yeni birim ID..."
          />
          <Input
            type="text"
            value={newUnit}
