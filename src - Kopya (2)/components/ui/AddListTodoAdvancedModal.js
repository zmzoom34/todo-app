...
function AddListTodoAdvancedModal({ isOpen, onClose, listName, db, user, activeTab, selectedGroupId }) {
    const availableFieldTypes = [
        ...
        { id: "date", type: "date" },
        { id: "checkbox", type: "checkbox" }, // Added checkbox type
    ];
        ...
        setFormValues(prev => ({
            ...
            fieldConfigs: {
                ...
                [fieldId]: { 
                    type, 
                    label: "", 
                    options: type === "select" ? [] : null, // No options for checkbox
                },
            },
        }));
    };
    ...
    const validateForm = () => {
        ...
        formValues.selectedFields.forEach(fieldId => {
            ...
            // Optional: Add validation for select options if needed
            if (config.type === "select" && (!config.options || config.options.length === 0)) {
                newErrors[fieldId] = "At least one option is required for select fields";
            }
        });
        ...
        try {
            ...
            const selectedFieldDefinitions = formValues.selectedFields.map(fieldId => {
                ...
                return {
                    ...
                    label: config.label,
                    type: config.type,
                    options: config.type === "select" ? config.options : null, // Only for select
                                ...
                                return (
                                    <div key={fieldId} className="flex flex-col border p-2 rounded-md">
                                        ...
                                        <div className="mt-2 space-y-2">
                                            ...
                                            {/* No additional input for checkbox since it doesn’t need options */}
                                        </div>
                                    </div>
                                );
                            })}
