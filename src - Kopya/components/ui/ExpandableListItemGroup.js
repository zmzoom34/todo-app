...
// Modified SortableTodoItem component with delete button
// function SortableTodoItem({ todo, toggleTodoCompletion, onDelete }) {
//   const {
//     attributes,
//     listeners,
//     setNodeRef,
...
function ExpandableListItemGroup({
    list,
    onEdit,
    onDelete,
    user,
  ...
  const updateTodoText = async (listId, todoId, newText) => {
    try {
      if (!user) {
        console.error("User not authenticated");
        return;
  ...
  const handleDeleteTodo = async (todoId) => {
    ...
    if (confirmed) {
      await deleteTodoFromList(db, user, list.id, todoId, selectedGroupId);
    }
  };
  ...
  const handleDeleteList = async (listId) => {
    ...
    if (ans) {
      try {
        ...
        const listDocRef = doc(db, "groups", selectedGroupId, "lists", listId);
...
export default ExpandableListItemGroup;
